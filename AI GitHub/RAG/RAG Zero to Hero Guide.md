...
- Indexing: Tạo chỉ mục cho dữ liệu để dễ dàng truy xuất.

- Retrieval: Truy xuất thông tin liên quan từ nguồn dữ liệu.

- Augmentation: Kết hợp thông tin truy xuất với truy vấn ban đầu.

- Generation: Sử dụng LLM để tạo ra câu trả lời dựa trên thông tin đã kết hợp.
...
flowchart TD
    ...
    E --> F[6️⃣ Augmented Prompt<br/>Tạo prompt tăng cường]
    F --> G[7️⃣ Generation<br/>Sinh phản hồi bằng LLM]
    G --> H[8️⃣ Post-processing<br/>Xử lý đầu ra]
...
- Augmented Prompt (Tạo prompt tăng cường): Kết hợp truy vấn của người dùng với các đoạn văn bản truy xuất được để tạo thành prompt đầu vào cho mô hình ngôn ngữ lớn.
...
#### 5.1 📚 Thuật ngữ: Retrieval (truy xuất)
##### 🔤 Phát âm & Phiên âm
Từ vựng: Retrieval
...
##### 🧾 Định nghĩa đơn giản
Retrieval là quá trình tìm kiếm và lấy thông tin phù hợp từ một kho dữ liệu để sử dụng trong một nhiệm vụ cụ thể. Trong AI, đặc biệt là trong RAG, retrieval đề cập đến việc truy xuất các tài liệu hoặc dữ liệu liên quan từ một cơ sở dữ liệu để hỗ trợ quá trình tạo ra câu trả lời chính xác và có căn cứ.
...
---
#### 5.2 📘 Thuật ngữ: Augmented (tạo prompt tăng cường)
##### 🔤 Phát âm & Phiên âm
Từ vựng: Augmented
...
---
#### 5.3 📘 Thuật ngữ: Extract (Trích xuất dữ liệu)
##### 🔤 Phát âm & Phiên âm
Từ vựng: Extract
...
---
#### 5.4 📘 Thuật ngữ: Chunking (phân đoạn)
##### 🔤 Phát âm & Phiên âm
Từ vựng: Chunking
...
---
#### 5.5 📘 Thuật ngữ: Embedding (nhúng, vector hóa dữ liệu)
##### 🔤 Phát âm & Phiên âm
Từ vựng: Embedding
...
---
#### 5.6 📘 Thuật ngữ: Indexing (tạo chỉ mục lưu trữ vector)
##### 🔤 Phát âm & Phiên âm
Từ vựng: Indexing
